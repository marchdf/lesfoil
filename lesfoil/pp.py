...
def main():
    ...
    for xloc in ut.cord_locations():
        ...
        if rank == 0:
            ...
            p = plt.plot(xnml, ynml, lw=2, color=cmap[1], label="normal")
            ...
            p = plt.plot(xi * np.ones(yi.shape), yi, lw=2, color=cmap[0])
            p = plt.plot(xp, yp, lw=2, color=cmap[1], label="Rotated airfoil")
            p = plt.plot(
                0.0,
                0.0,
                lw=0,
